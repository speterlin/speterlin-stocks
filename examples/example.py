...
# NO BACKTESTING on airs type since AI in past data is irreflective (genai google gemini-pro always updated)
portfolio_airs_test = {
...
# NO BACKTESTING on tngaia type since AI in past data is irreflective (genai google gemini-pro always updated)
...
# tickers_to_avoid = {**{ticker: {'reason': 'stock 1d and granular market data (issue with date column) not on FMP', 'good_after_date': datetime.strptime('2029_06_06 13:00:00', '%Y_%m_%d %H:%M:%S')} for ticker in [
#     'DMIIR', # tilupccu backtest issue data retrieval 2025-12-18 (1d works in backtest but granular next day 2025-12-19 shows up float("NaN"), when checking both 1d and granular don't work - fmp deletes ticker from data? no apple/yfinance data)
# ]}, **tickers_to_avoid}
...
portfolio['open'].sort_values('current_roi', inplace=False, ascending=False)[['buy_date', 'buy_price', 'balance', 'rank_rise_d', 'current_date', 'current_price', 'current_roi', 'other_notes']]
