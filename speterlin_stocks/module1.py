...
def get_crunchbase_data_for_ticker(ticker, permalink_original, **params): # company is lowercase string like 'asana' # old name - get_crunchbase_user_data_for_company_with_ticker() # apply same logic in save_usa_alpaca_tickers_fmp_or_gf_data() function with its own counter (separate from yahoo finance counter) # maybe refactor and remove resp.status_code from return [data, resp.status_code]
    print("<<< " + ticker + " >>>")
...
def alpaca_trade_ticker(ticker, side, usd_invest=None, quantity=None, price=None, paper_trading=True, open_time=5, other_notes=None): # added open_time due to retrying paper order 'FGF' error 2022-12-13 "Partially filled" listed as "~Filled" - in past testing conditions 0 "~Filled" positions which means open_order gets processed immediately but retrying paper orders are filled during market hours different testing conditions # maybe refactor side logic as precautionary in case alpaca api changes from normal "buy" and "sell" # fetching price and calculating quantity in method since better to get price as close to executing alpaca_api.submit_order as possible even though when not back_testing using same price for data analysis and trading (Alpaca), unlike in crypto - CoinGecko (which is exchange volume-weighted) for data analysis and Binance for trading
    ...
    # trade_notes = 'Filled' if order.status == 'filled' else 'Not filled' if order.status == 'accepted' and not float(order.filled_qty) else 'Partially filled' if order.status == 'partially_filled' and float(order.filled_qty) else "~Filled"
...
def update_portfolio_postions_back_testing(portfolio, stop_day, end_day, **params): # maybe refactor name to update_portfolio_open_postions_back_testing, maybe integrate with portfolio_trading section so only have one function for both functions
    ...
    for ticker in portfolio['open'].index:
        ...
        ticker_data = _fetch_data(get_ticker_data_fmp, params={'ticker': ticker, 'start_datetime': stop_day, 'end_datetime': stop_day}, error_str=" - No " + "1d" + " ticker data for: " + ticker + " from datetime: " + str(stop_day) + " to datetime: " + str(stop_day), empty_data=pd.DataFrame())
        ...
        if ticker_data_granular.empty or ticker_data.empty: # 'market_data' not in ticker_data_granular or not ticker_data_granular['market_data']['market_cap']['usd']: # remove granular from error_str
            ...
            if tickers_with_stock_splits['apply_corrections'] and (ticker in tickers_with_stock_splits['stock_splits']['symbol'].values) and not (math.isclose(ticker_data_granular.iloc[-1]['close'], ticker_data.iloc[-1]['close'], rel_tol=0.20)): # and (ticker not in tickers_with_stock_splits['avoid_splitting']) # ticker in tickers_with_stock_splits: # and (stop_day.date() < tickers_with_stock_splits[ticker]['ex_date'].date()) # and (ticker not in tickers_to_avoid_splitting) add back if Yahoo Finance decides to update (hourly price data to match past daily price data) some tickers and not others # because yahoo finance api updates past daily price data but not hourly price data following a stock split, maybe refactor if notice a change, assuming for now that it remains this way
                ...
                # else:
                #     print("Error retreiving market data FMP for ticker: " + ticker + " or potential stock split already reflected on FMP" + " on date: " + stop_day.strftime('%Y-%m-%d'))
...
def get_tickers_to_avoid(df_usa_alpaca_tickers_of_the_day, end_day): # probably refactor
    if (not type(end_day) is datetime):
        print("End day must be of type datetime")
        return {}
...
def update_portfolio_buy_and_sell_tickers(portfolio, tickers_to_buy, tickers_to_sell, tickers_to_avoid, stop_day, paper_trading, back_testing):
    ...
    for ticker, rank_rise_d in tickers_to_buy: # buy at end of day (13:00PST),when retrieve ticker information # ticker, rank_rise_d = ticker_and_rank_rise_d[0], ticker_and_rank_rise_d[1] # process takes about ~0.7 seconds to buy each ticker (I think maybe analysis was done with coin)
        if (portfolio['balance']['usd'] >= USD_INVEST_MIN) and (ticker not in tickers_to_avoid): # and (ticker not in portfolio['sold']['ticker'].values) # can add max open positions and waiting list to reflect real trading, don't allow repeat buys of same company since repeat buys reflect long-term investment in company / more exposure to single asset
            ...
            # ticker_data_granular = _fetch_data(get_ticker_data_granular_fmp, params={'ticker': ticker, 'start_datetime': stop_day, 'end_datetime': stop_day, 'interval': '1hour'}, error_str=" - No " + "1hour" + " ticker data for: " + ticker + " from datetime: " + str(stop_day) + " to datetime: " + str(stop_day), empty_data=pd.DataFrame())
...
def get_sp500_ranked_tickers_by_slickcharts() :
    ...
    for idx,row in enumerate(table.find_all('tr')[1:]):
        ...
        weight, price, price_change = float(tds[3].text.strip().replace('%',''))/100, float(tds[4].text.strip().replace(',','').split("Â\xa0Â\xa0")[-1]), float(tds[6].text.strip().replace('%','').replace('(','').replace(')',''))/100
...
def check_for_basic_errors_and_set_general_params_for_run_portfolio(portfolio, start_day, end_day, paper_trading, back_testing, back_running_allowance=17.5, **params):
    ...
    if (back_testing and not paper_trading) or (not back_testing and (stop_day < datetime.now() - timedelta(hours=back_running_allowance))): # back_running_allowance=11 for airs for some reason # 6:30am (new data arrives - except weekends & holidays) - 01:00pm previous day = 17.5 # assuming datetime input is always in 13:00:00 and run after market close of the stop_day # (stop_day.date() != datetime.now().date()) # maybe refactor here and below, allow 24 hours or more custom (especially if run update_portfolio_buy_and_sell_tickers() more than once per day after market hours as it is now or if account for running on weekends) # could also be more like in crypto.py or add full datetime comparison # here and below no need to check for (end_day.date() <= start_day.date()) since while loop won't execute (since stop_day is always > start_day will be greater than end_day)
        ...
        return ["Error", portfolio, None, None, None, None, None, None, None]
    ...
    return ["Success", portfolio, DAYS, end_day, start_day, stop_day, tickers_with_stock_splits, count, usa_holidays]

# Not merging run_portfolio_fmpr/zr (previous run_portfolio_zr) into single function since even though it appears that sometimes only difference in algorithm functions is how to pick tickers there can be nuiances like if portfolio['constants']['up_down_move'] > 4: in run_portfolio_fmpr() and potentially more between portfolios
            ...
            if fmpr_sell or (portfolio['balance']['usd'] >= portfolio['constants']['usd_invest_min']):
                ...
                if not (df_tickers_interval_start.empty or df_tickers_interval_stop.empty): # df_tickers_interval_stop.empty
                    ...
                    if (tickers_to_buy or tickers_to_sell):
                        portfolio = update_portfolio_buy_and_sell_tickers(portfolio=portfolio, tickers_to_buy=tickers_to_buy, tickers_to_sell=tickers_to_sell, tickers_to_avoid=get_tickers_to_avoid(df_tickers_interval_stop, stop_day), stop_day=stop_day, paper_trading=paper_trading, back_testing=back_testing)
        else:
            ...
            if portfolio['balance']['usd'] >= portfolio['constants']['usd_invest_min']:
                ...
                if not (df_tickers_interval_start.empty or df_tickers_interval_stop.empty): # df_tickers_interval_stop.empty
                    ...
                    for top_interval_loser_ticker, last_price_change in tickers_with_last_price_change.most_common()[-top_interval_losers:]: # maybe refactor (in testing didn't work out as well and more volatile) so top_interval_losers are ordered by most negative first: sorted(tickers_with_last_price_change.items(), key=lambda pair: pair[1], reverse=False)[:top_interval_losers]
                        ...
                        eps_fy_or_ttm = 'Basic EPS (FY)' if df_tickers_interval_stop.loc[top_interval_loser_ticker, 'Basic EPS (FY)'] else 'Basic EPS (TTM)' if 'Basic EPS (TTM)' in df_tickers_interval_stop else 'Basic EPS (FY)' # probably refactor last else statement
                        peer_tickers_basic_eps_filtered = [peer_ticker_basic_eps for peer_ticker_basic_eps in df_tickers_last_quality_data[df_tickers_last_quality_data['Industry'] == industry][eps_fy_or_ttm] if peer_ticker_basic_eps] # df_tickers_interval_stop # maybe refactor and change to unfiltered, assume that if no Basic EPS that coompany is not robust # maybe refactor name to peer_companies hard to accomdate logic and grammar, assumed since checking industry that only companies
                        ...
                        pb = df_tickers_last_quality_data.loc[top_interval_loser_ticker, 'P/B (TTM)'] if df_tickers_last_quality_data.loc[top_interval_loser_ticker, 'P/B (TTM)'] else float("NaN") # ensure that it's a float("NaN") if None value # df_tickers_interval_stop # top_interval_loser_ticker is in df_tickers_interval_stop as per previous for loop (top_interval_loser_ticker in df_tickers_interval_stop.index) and
                        basic_eps = df_tickers_interval_stop.loc[top_interval_loser_ticker, eps_fy_or_ttm] if df_tickers_interval_stop.loc[top_interval_loser_ticker, eps_fy_or_ttm] else float("NaN") # df_tickers_interval_stop.loc[top_interval_loser_ticker, 'Basic EPS (TTM)'] if 'Basic EPS (TTM)' in df_tickers_interval_stop else df_tickers_interval_stop.loc[top_interval_loser_ticker, 'Basic EPS (FY)'] if 'Basic EPS (FY)' in df_tickers_interval_stop else float("NaN")
                        if peer_tickers_basic_eps_filtered and ((len([peer_ticker_basic_eps_filtered for peer_ticker_basic_eps_filtered in peer_tickers_basic_eps_filtered if peer_ticker_basic_eps_filtered >= 0]) / len(peer_tickers_basic_eps_filtered)) >= 0.5) and (pb <= PB_LIMIT) and (basic_eps >= EPS_LIMIT) and (top_interval_loser_ticker not in portfolio['open'].index): # maybe refactor industry considered strong if > 50% of peer tickers have eps >= 0 # maybe add UP_MOVE for last_price_change
            ...
            if mmtv_sell or (portfolio['balance']['usd'] >= portfolio['constants']['usd_invest_min']):
                ...
                if not (df_tickers_interval_start.empty or df_tickers_interval_stop.empty): # df_tickers_interval_stop.empty
                    ...
                    for ticker in df_tickers_interval_stop[:limit_tickers].index: # (df_tickers_interval_stop if stop_day.date() < datetime.strptime('2020-05-08', '%Y-%m-%d').date() else df_tickers_interval_stop[df_tickers_interval_stop['Market Cap'] >= 2e9]).index: # df_tickers_interval_stop # if want to deal only with Mega, Large, Mid-Cap companies since smaller companies could cause complications with exchange listings etc. (but not on alpaca), S&P 500 tickers from 04/24/2020 until 05/08/2020
                        trading_volume_stop_day = float(df_tickers_interval_stop.loc[ticker, 'Volume']) if df_tickers_interval_stop.loc[ticker, 'Volume'] else float("NaN") # trading_volume_on_interval_day if interval_day.date() == stop_day.date() else float("NaN")
                        ...
                        while interval_day.date() <= stop_day.date():
                            ...
                            try: # if not df_tickers_interval_day.empty else  None
                                trading_volume_on_interval_day = float(df_tickers_interval_day.loc[ticker, 'Volume'])
                            except:
    ...
    while stop_day.date() <= end_day.date():
        if not (portfolio['open']['current_date'] >= stop_day).any(): # in case re-run existing portfolio over same days to avoid back running (and conserve time): avoid selling existing tickers incorrectly to TSL (too early) due to tsl_max_price set on future day or selling/buying existing/new tickers incorrectly with algorithm logic # assuming datetime is always in 13:00:00
            ...
            if random_sp500_sell or (portfolio['balance']['usd'] >= portfolio['constants']['usd_invest_min']):
                ...
                if (tickers_to_buy or tickers_to_sell):
                    portfolio = update_portfolio_buy_and_sell_tickers(portfolio=portfolio, tickers_to_buy=tickers_to_buy, tickers_to_sell=tickers_to_sell, tickers_to_avoid={}, stop_day=stop_day, paper_trading=paper_trading, back_testing=back_testing) # not doing get_tickers_to_avoid(df_tickers_interval_stop, stop_day) since only buying sp500 tickers, need to refactor once I get a sp500 historical source and call it every day of the backtest
        else:
            print("skipping " +  str(stop_day) + " since portfolio has already run on this date")
        ...
        if not (portfolio['open']['current_date'] >= stop_day).any(): # in case re-run existing portfolio over same days to avoid back running (and conserve time): avoid selling existing tickers incorrectly to TSL (too early) due to tsl_max_price set on future day or selling/buying existing/new tickers incorrectly with algorithm logic # assuming datetime is always in 13:00:00
            ...
            if portfolio['balance']['usd'] >= portfolio['constants']['usd_invest_min']: # tngaia_sell or - unlikely that a top-n gainer from a new day is going to show up from a previous day and be rated as a sell
                ...
                if not (df_tickers_interval_stop.empty or df_tickers_daily_gainers.empty):
                    ...
                    if (tickers_to_buy or tickers_to_sell):
                        portfolio = update_portfolio_buy_and_sell_tickers(portfolio=portfolio, tickers_to_buy=tickers_to_buy, tickers_to_sell=tickers_to_sell, tickers_to_avoid=get_tickers_to_avoid(df_tickers_interval_stop, stop_day), stop_day=stop_day, paper_trading=paper_trading, back_testing=back_testing) # get_tickers_to_avoid(df_tickers_interval_stop, stop_day) precautionary
        else:
            print("skipping " +  str(stop_day) + " since portfolio has already run on this date")
        stop_day = stop_day + timedelta(days=1)
        while stop_day.weekday() > 4 or (stop_day.replace(hour=0, minute=0, second=0, microsecond=0) in usa_holidays): # interval_start_date/stop_day in usa_holidays # stop_day = (stop_day + timedelta(days=1)) if (stop_day.weekday() < 4) else (stop_day + timedelta(days=7-stop_day.weekday())) # to avoid weekends, refactor to avoid holidays (2020-05-25)
...
def run_portfolio_senate_trading(portfolio, start_day=None, end_day=None, senate_trading_sell=True, paper_trading=True, back_testing=False, add_pauses_to_avoid_unsolved_error={'engaged': False, 'time': 240, 'days': 15}, **params): # maybe refactor and add sp500 and month to name
    ...
    if (not type(portfolio['constants']['up_down_move']) is int) or not (1 <= portfolio['constants']['up_down_move'] <= 50): # [0]is higher rank threshold [1] is lower rank threshold
        print("Error up/down move constant is not an int or not within 1-50 inclusive")
        return portfolio
...
def run_portfolio_sma_mm(portfolio, start_day=None, end_day=None, sma_mm_sell=True, paper_trading=True, limit_tickers=1000, **params):
    ...
    if stop_day.hour > 18: # assuming only called / running on market days / weekdays
        ...
        if tickers_to_sell:
            portfolio = update_portfolio_buy_and_sell_tickers(portfolio=portfolio, tickers_to_buy=[], tickers_to_sell=tickers_to_sell, tickers_to_avoid={}, stop_day=stop_day, paper_trading=paper_trading, back_testing=False)
        return portfolio
    ...
    print("Execution time: " + str(time.time() - start_time))
    if (tickers_to_buy or tickers_to_sell):
        portfolio = update_portfolio_buy_and_sell_tickers(portfolio=portfolio, tickers_to_buy=tickers_to_buy, tickers_to_sell=tickers_to_sell, tickers_to_avoid=get_tickers_to_avoid(df_tickers_to_iterate, interval_stop_date), stop_day=stop_day, paper_trading=paper_trading, back_testing=False)
    return portfolio
...
def sleep_until_minute_mark_within_hour(paper_trading, paper_trading_on_used_account, portfolio_usd_value_negative_change_from_max_limit, portfolio_current_roi_restart, download_and_save_tickers_data, minutes=30.0): # maybe refactor to allow for different sleeping times (i.e. 30, 60 min) # maybe refactor name to sleep_until_minute_mark_within_hour_while_trading() # , buying_disabled
...
def retry_alpaca_open_orders_in_portfolio(portfolio, alpaca_open_orders, open_order_price_difference=0.15, open_time=15): # paper_trading
    for open_order in alpaca_open_orders:
        ...
        quantity = original_quantity - executed_quantity # assuming (and should be based on code), that original_quantity is same as portfolio['open'].loc[ticker, 'balance'], no need for precautionary check IMO # position, 'position'
        ...
        if not df_matching_open_positions.empty:
            ...
            buy_price = portfolio['open'].loc[ticker, 'buy_price']
            portfolio['open'].loc[ticker, 'trade_notes'] = "Partially filled" if quantity != original_quantity else portfolio['open'].loc[ticker, 'trade_notes'] # maybe refactor, here and below easy correction here, rare situation like with ELC on 2026-07-15
            ...
            if (incremental_usd_invest <= portfolio['balance']['usd']) and not (math.isclose(price, buy_price, rel_tol=0.05)) and ((price - buy_price) / buy_price <= open_order_price_difference): # refactor can make open_order_price_difference a parameter also not sure if necessary, can be sl/2 # maybe refactor here and below and put back in check for not np.isnan(price) since covered in expression: (price - buy_price) / buy_price <= open_order_price_difference which will be False if price == nan # maybe refactor possible edge case 2021-07-28 # here and below no need to retry order if the price hasn't changed much
        ...
        if not df_matching_sold_positions.empty: # maybe refactor and add precautionary check - shouldn't have to check len() == 1 since symbol, trade_notes and order_time time frame (+/- 10 minutes)
            ...
            portfolio['sold'].loc[idx, 'trade_notes'] = "Partially filled" if quantity != original_quantity else portfolio['sold'].loc[idx, 'trade_notes'] # maybe refactor, easy correction here, rare situation like with ELC on 2026-07-15
            last_trade_data = _fetch_data(alpaca_api.get_latest_trade, params={'symbol': ticker}, error_str=" - No last trade from Alpaca for ticker: " + ticker + " on: " + str(datetime.now()), empty_data = {})
            price = last_trade_data.price if last_trade_data else float("NaN")
            if not (math.isclose(price, sell_price, rel_tol=0.05)) and ((price - sell_price) / sell_price >= -open_order_price_difference): # not np.isnan(price) and # no use in retrying order if the price hasn't changed
                resp = _fetch_data(alpaca_api.cancel_order, params={'order_id': order_id}, error_str=" - Alpaca cancel order error for ticker: " + ticker + " and order ID: " + str(order_id) + " on: " + str(datetime.now()), empty_data={})
                time.sleep(open_time)
                canceled_order = _fetch_data(alpaca_api.get_order, params={'order_id': order_id}, error_str=" - Alpaca get order error for ticker: " + ticker + " and order ID: " + str(order_id) + " on: " + str(datetime.now()), empty_data={})
...
def portfolio_trading(portfolio, paper_trading=True, paper_trading_on_used_account=False, portfolio_usd_value_negative_change_from_max_limit=-0.30, portfolio_current_roi_restart={'engaged': False, 'limit': 0.075}, download_and_save_tickers_data={'engaged': False, 'fmp_paid_data': False}, sleep_interval_during_market_hours=240.0, min_start_and_end_check_assets_etc_during_market_hours=[41,45]): # refactor to mimick run_portfolio_rr # short=False, possibly add short logic # maybe refactor buying_disabled=False as default and then change within function (or if specified) based on certain conditions
    # global portfolio_account, twilio_client, twilio_phone_from, twilio_phone_to
    DAYS = portfolio['constants']['days']
    STOP_LOSS = portfolio['constants']['sl']
    TRAILING_STOP_LOSS_ARM, TRAILING_STOP_LOSS_PERCENTAGE = portfolio['constants']['tsl_a'], portfolio['constants']['tsl_p']
    ...
    while True:
        ...
        if (datetime.now(eastern).weekday() < 5) and (datetime.now(eastern).replace(hour=0, minute=0, second=0, microsecond=0).replace(tzinfo=None) not in usa_holidays): # .replace(tzinfo=None) since usa_holidays holds datetime objects without timezone (ie datetime.datetime(2020, 11, 26, 0, 0))
            ...
            if (datetime.now(eastern).hour >= 9) and (datetime.now(eastern).hour < 16): # stocks: maybe refactor to get total time in seconds (from beginning of day) instead of like this, so can be more exact (i.e. start at 9:30am EST so no beforehours) # no afterhours trading since risky due to less liquidity, doesn't reflect normal market conditions # maybe refactor bug if run on Friday and data not downloaded until
                ...
                if not paper_trading_on_used_account and (datetime.now(eastern).minute >= min_start_and_end_check_assets_etc_during_market_hours[0]) and (datetime.now(eastern).minute < min_start_and_end_check_assets_etc_during_market_hours[1]): # >= 30 and < 34 # running at 41-45min mark since market opens on the half hour (6:30am PST/9:30am EST) and don't want to conflict with crypto potential processing build up which runs every on the half hour
