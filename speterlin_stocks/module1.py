            ...
            if portfolio['balance']['usd'] >= portfolio['constants']['usd_invest_min']:
                ...
                if not (df_tickers_interval_start.empty or df_tickers_interval_stop.empty): # df_tickers_interval_stop.empty
                    ...
                    for top_interval_loser_ticker, last_price_change in tickers_with_last_price_change.most_common()[-top_interval_losers:]: # maybe refactor (in testing didn't work out as well and more volatile) so top_interval_losers are ordered by most negative first: sorted(tickers_with_last_price_change.items(), key=lambda pair: pair[1], reverse=False)[:top_interval_losers]
                        ...
                        peer_tickers_basic_eps_filtered = [peer_tickers_basic_eps for peer_tickers_basic_eps in df_tickers_last_quality_data[df_tickers_last_quality_data['Industry'] == industry][eps_fy_or_ttm] if peer_tickers_basic_eps] # df_tickers_interval_stop # maybe refactor and change to unfiltered, assume that if no Basic EPS (TTM) that the coompany is not robust # maybe refactor name to peer_company and peer_companies hard to accomdate logic and grammar, assumed since checking industry that only companies
                        ...
                        if peer_tickers_basic_eps_filtered and ((len([basic_eps for basic_eps in peer_tickers_basic_eps_filtered if basic_eps >= 0]) / len(peer_tickers_basic_eps_filtered)) >= 0.5) and (pb <= PB_LIMIT) and (basic_eps >= EPS_LIMIT) and (top_interval_loser_ticker not in portfolio['open'].index): # ticker # [0] # maybe refactor industry considered strong if > 50% of similar companies have eps >= 0 # maybe add UP_MOVE for last_price_change
